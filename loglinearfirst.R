...
linearMod <- lm(Firstwave1 ~ Day, data=LINEARKUWAIT1)
print(linearMod)
summary(linearMod)
...
lmMod <- lm(Firstwave1 ~ Day, data=trainingData)  # build the model
...
Secondwave.test <- testData$Firstwave1
par(mfrow=c(1,2))
...
library(ggplot2)
theme_set(theme_bw())
pl <- ggplot(LINEARKUWAIT1) + geom_point(aes(x=Day, y=Firstwave1), size=2, colour="#993399") + 
...
library(DAAG)
cvResults <- suppressWarnings(CVlm(data=LINEARKUWAIT1, form.lm=Firstwave1 ~ Day, m=5, dots=FALSE, seed=29, legend.pos="topleft",  printit=FALSE));  # performs the CV
attr(cvResults, 'ms')   
scatter.smooth(x=LINEARKUWAIT1$Day, y=LINEARKUWAIT1$Firstwave1, main="Firstwave1 ~ Day")  # scatterplot 
plot(linearMod$residuals, pch = 16, col = "red")
ggplot(LINEARKUWAIT1, aes(x = Day, y = Firstwave1)) +
  geom_point() +
  stat_smooth()
cor(LINEARKUWAIT1$Day, LINEARKUWAIT1$Firstwave1)
ggplot(LINEARKUWAIT1, aes(Day, Firstwave1)) +
  geom_point() +
  stat_smooth(method = lm)
confint(linearMod)
# RESIDUAL STANDARD ERROR
sigma(linearMod)*100/mean(LINEARKUWAIT1$Firstwave1)
par(mfrow=c(2,2))
plot(linearMod)
par(mfrow=c(1,1))
